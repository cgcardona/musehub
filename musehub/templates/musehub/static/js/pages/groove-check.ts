/**
 * groove-check.ts — Groove check page module.
 *
 * Reads config from the #page-data JSON element:
 *   { page: "groove-check", repoId, base }
 */
...
interface GrooveCheckData {
  entries: GrooveEntry[];
  flaggedCommits: number;
  worstCommit: string | null;
  commitRange: string;
  threshold: number;
  totalCommits: number;
}
...
function statusBadge(status: string): string {
  const cls: Record<string, string> = { OK: 'status-ok', WARN: 'status-warn', FAIL: 'status-fail' };
  return '<span class="status-badge ' + (cls[status] ?? '') + '">' + window.escHtml(status) + '</span>';
}

function renderGrooveChart(entries: GrooveEntry[], width: number, height: number): string {
  if (!entries || !entries.length) return '<p class="loading">No data to chart.</p>';
  ...
  const STATUS_COLOR: Record<string, string> = { OK: '#3fb950', WARN: '#f0883e', FAIL: '#f85149' };
  ...
  entries.forEach((e, i) => {
    ...
    bars += '<rect x="' + x + '" y="' + y + '" width="' + barW + '" height="' + barH
      ...
      + window.escHtml(e.commit) + ' score: ' + e.grooveScore.toFixed(4) + ' (' + e.status + ')'
      + '</title></rect>';
    if (entries.length <= 14 || i % 2 === 0) {
      bars += '<text x="' + (x + barW / 2) + '" y="' + (height - 6) + '" font-size="9" fill="#8b949e"'
        ...
        + window.escHtml(e.commit.substring(0, 6)) + '</text>';
    }
  });
...
async function loadGrooveCheck(threshold: number, limit: number): Promise<void> {
  const contentEl = document.getElementById('content');
  if (contentEl) contentEl.innerHTML = '<p class="loading">Analysing groove\u2026</p>';
  try {
    let url = '/repos/' + _gcRepoId + '/groove-check?limit=' + limit;
    if (threshold) url += '&threshold=' + threshold;
    const data = (await window.apiFetch(url)) as GrooveCheckData;
    ...
    const worstHtml = data.worstCommit
      ? '<a href="' + _gcBase + '/commits/' + data.worstCommit + '" style="font-family:monospace">'
        + window.escHtml(data.worstCommit) + '</a>'
    ...
    const tableRows = entries.length === 0
      ...
      : entries.map(e =>
          '<tr>'
          + '<td style="font-family:monospace"><a href="' + _gcBase + '/commits/' + e.commit + '">'
          + window.escHtml(e.commit) + '</a></td>'
          + '<td>' + statusBadge(e.status) + '</td>'
          + '<td>' + e.grooveScore.toFixed(4) + '</td>'
          + '<td>' + (e.driftDelta > 0 ? '+' : '') + e.driftDelta.toFixed(4) + '</td>'
          + '<td style="color:#8b949e">' + window.escHtml(e.track ?? '—') + '</td>'
      ...
      <div class="card">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;flex-wrap:wrap">
          ...
          <label style="font-size:13px;color:#8b949e;display:flex;align-items:center;gap:6px">
            Limit:
            <select id="limit-sel" onchange="window._gcLoad(document.getElementById('thr-inp').value, this.value)">
              ${[5, 7, 10, 20, 30].map(n =>
          ...
          <label style="font-size:13px;color:#8b949e;display:flex;align-items:center;gap:6px">
            ...
            <input id="thr-inp" type="number" step="0.01" min="0.01" max="1" value="${threshold}"
              ...
              onchange="window._gcLoad(this.value, document.getElementById('limit-sel').value)" />
          </label>
        </div>
        <div class="groove-summary-grid">
          <div class="groove-metric-card">
            <span class="groove-metric-label">Range</span>
            <span class="groove-metric-value" style="font-size:13px;font-family:monospace">${window.escHtml(data.commitRange)}</span>
          </div>
          <div class="groove-metric-card">
      ...
      <div class="card">
        ...
        <div style="overflow-x:auto">
          <table class="groove-table">
            <thead>
              <tr>
                <th>Commit</th><th>Status</th><th>Groove Score</th>
                <th>Drift Delta</th><th>Track</th><th>MIDI Files</th>
              </tr>
            </thead>
...
export function initGrooveCheck(data: PageData): void {
  _gcRepoId = String(data['repoId'] ?? '');
  _gcBase   = String(data['base'] ?? '');

  if (window.initRepoNav) void window.initRepoNav(_gcRepoId);

  (window as unknown as Record<string, unknown>)['_gcLoad'] = (thr: string | number, lim: string | number) => {
    void loadGrooveCheck(Number(thr), Number(lim));
  };

  void loadGrooveCheck(0.1, 10);
}
