/**
 * tempo.ts — Tempo analysis page module.
 *
 * Reads config from the #page-data JSON element:
 *   { page: "tempo", repoId, ref, base }
 */

type PageData = Record<string, unknown>;

function tempoChangeSvg(changes: Array<{ beat: number; bpm: number }>, baseBpm: number): string {
...
function stabilityBar(stability: number): string {
...
export function initTempo(data: PageData): void {
  ...
  void (async () => {
    try {
      interface TempoData {
        ...
        timeFeel: string;
        stability: number;
        tempoChanges: Array<{ beat: number; bpm: number }>;
      }
      interface TempoResp { data: TempoData }
      const resp = (await window.apiFetch(
        '/repos/' + repoId + '/analysis/' + encodeURIComponent(ref) + '/tempo',
      )) as TempoResp;
      const d = resp.data;
        ...
        <div class="card">
          ...
          <div class="meta-row" style="margin-bottom:16px">
            ...
            <div class="meta-item">
              ...
              <span class="meta-value">${window.escHtml(d.timeFeel)}</span>
            </div>
            <div class="meta-item">
          ...
          <p style="font-size:12px;color:#8b949e;margin-top:12px">
            ...
            <code style="font-size:11px;background:#0d1117;padding:2px 6px;border-radius:4px">
              /api/v1/repos/${repoId}/analysis/${encodeURIComponent(ref)}/tempo
            </code>.
          </p>
        </div>`;
    } catch (e: unknown) {
